...
impl ColorPickerSaturation {
    ...
    fn draw_gradient(&self, ctx: &Context<Self>) {
        if let (Some(canvas), Some(context)) = (self.canvas_ref.cast::<HtmlCanvasElement>(), &self.context) {
            ...
            gradient.add_color_stop(0.0, &format!("hsl({}, 0%, {}%)", ctx.props().hue, ctx.props().light * 100.0)).unwrap();
            gradient.add_color_stop(1.0, &format!("hsl({}, 100%, {}%)", ctx.props().hue, ctx.props().light * 100.0)).unwrap();
