...
impl Component for ColorPickerWheel {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        Self {
            ...
            resize_callback: Closure::wrap(Box::new(move || {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        html! {
            <div style={format!("position:relative")} class="w-full h-full aspect-square" > //style={format!("position: relative; height: {}px; width: {}px;", ctx.props().size.to_string(), ctx.props().size.to_string())}>
                <canvas
