//! Syntax highlighting support for the code editor
//!
//! This module provides syntax highlighting using the syntect library.
//! Enable the `syntax-highlighting` feature to use this module.
...
mod highlighter {
    ...
    impl SyntaxHighlighter {
        ...
        fn get_theme(theme_name: &str) -> &'static Theme {
            ...
            THEME_SET.themes.get(theme_key).unwrap_or_else(|| {
                if is_dark {
                    THEME_SET
                        .themes
        ...
        pub fn highlight_line(&self, line: &str) -> Html {
            ...
            if let Ok(ranges) = h.highlight_line(line, &SYNTAX_SET) {
                self.render_highlighted_ranges(&ranges)
            } else {
                html! { <span>{ line }</span> }
            }
        }

        fn render_highlighted_ranges(&self, ranges: &[(Style, &str)]) -> Html {
            let is_dark_theme = self
                .theme
                .name
                .as_ref()
                .map(|n| {
                    n.to_lowercase().contains("dark")
                        || n.to_lowercase().contains("monokai")
                        || n.to_lowercase().contains("ocean")
                })
                .unwrap_or(false);

            let spans: Vec<Html> = ranges
                .iter()
                .map(|(style, text)| {
                    let (r, g, b) = if is_dark_theme {
                        let brightness = (style.foreground.r as f32 * 0.299
                            + style.foreground.g as f32 * 0.587
                            + style.foreground.b as f32 * 0.114)
                            / 255.0;
...
// Fallback when syntax-highlighting is disabled
#[cfg(not(feature = "syntax-highlighting"))]
mod fallback {
    use yew::prelude::*;

    /// Fallback function when syntax-highlighting is disabled
    pub fn highlight_code_line(line: &str, _language: &str, _theme: &str, _for_diff: bool) -> Html {
        html! { <span>{ line }</span> }
    }
}
