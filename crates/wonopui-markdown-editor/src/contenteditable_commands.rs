use crate::utils::{document, window};
use wasm_bindgen::{prelude::Closure, JsCast};
use web_sys::{ClipboardEvent, FocusEvent, HtmlElement, InputEvent, KeyboardEvent};
use yew::prelude::*;
...
pub fn contenteditable_with_commands<T: Clone + PartialEq + 'static>(
    ...
    // Handle paste events - sanitize pasted content to plain text
    let on_paste = {
        let on_input = props.on_input.clone();
        let node_ref = node_ref.clone();
        Callback::from(move |e: web_sys::Event| {
            ...
            if let Ok(clipboard_event) = e.dyn_into::<ClipboardEvent>() {
                // Get plain text from clipboard
                if let Some(clipboard_data) = clipboard_event.clipboard_data() {
                    if let Ok(text) = clipboard_data.get_data("text/plain") {
                        // Sanitize the text - remove any HTML tags and normalize whitespace
                        let sanitized = text
                            .lines()
                            .map(|line| line.trim())
                            .collect::<Vec<_>>()
                            ...
                            if let Some(selection) = window().get_selection().ok().flatten() {
                                if selection.range_count() > 0 {
                                    if let Ok(range) = selection.get_range_at(0) {
                                        ...
                                        // Insert the text
                                        if let Ok(text_node) =
                                            document().create_text_node(&sanitized).dyn_into()
                                        {
                                            let _ = range.insert_node(&text_node);

                                            // Move cursor to end of inserted text
                                            let _ = range.set_start_after(&text_node);
                                        ...
                                        on_input.emit(element.inner_text());
                                    }
                                }
                            }
                        }
    ...
    let command_menu = if *show_commands {
        ...
        html! {
            <div
                class="bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded-md shadow-md max-h-64 overflow-y-auto min-w-[250px] animate-in fade-in-0 zoom-in-95 duration-200"
                style={menu_style}
            >
                <div class="p-1">
                    {
                        if filtered_options.is_empty() {
                            ...
                            filtered_options.iter().enumerate().map(|(idx, (value, _, display, icon))| {
                                ...
                                let item_class = if is_selected {
                                    ...
                                    "flex items-center gap-3 px-3 py-2 text-sm rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-700 cursor-pointer transition-all duration-150"
                                };
                                ...
                                html! {
                                    ...
                                    >
                                        if let Some(icon_html) = icon {
                                            <span class="w-4 h-4 flex items-center justify-center text-zinc-500 dark:text-zinc-400">
                                                { icon_html.clone() }
                                            </span>
                                        } else {
                                        ...
                                        if is_selected {
                                            <span class="text-xs text-zinc-500 dark:text-zinc-400 font-mono">{"↵"}</span>
                                        }
                                    </div>
                                }
                            }).collect::<Html>()
