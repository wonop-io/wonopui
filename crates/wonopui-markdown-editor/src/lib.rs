//! Block-based Markdown Editor component for WonopUI.
//!
//! This crate provides a rich text editor with block-based editing, drag-and-drop
//! reordering, undo/redo history, and markdown import/export.
...
pub use contenteditable_commands::{ContentEditableWithCommands, ContentEditableWithCommandsProps};
use editor_block::EditorBlock;
...
pub struct MarkdownEditorProps<T: BlockTrait> {
    ...
    #[prop_or_default]
    pub class: Classes,
    /// Initial content as a vector of blocks
    #[prop_or_default]
    ...
    #[prop_or_default]
    pub auto_focus: bool,
    /// Callback when a specific block is updated
    #[prop_or_default]
    pub update_block: Option<Callback<(usize, T)>>,
    /// Callback when the editor loses focus
    #[prop_or_default]
    pub on_blur: Option<Callback<FocusEvent>>,
    /// Show block action buttons (duplicate, delete, move)
    #[prop_or(false)]
    pub show_block_actions: bool,
}

/// A block-based markdown editor component
...
pub fn markdown_editor<T: BlockTrait>(props: &MarkdownEditorProps<T>) -> Html {
    ...
    let selected_blocks = use_state(|| HashSet::<usize>::new());
                    ...
                    if let Some(selection) = window().get_selection().ok().flatten() {
                        if selection.range_count() > 0 {
                            if let Ok(range) = selection.get_range_at(0) {
                                ...
                                if let Ok(text_node) = document().create_text_node("    ").dyn_into() {
    ...
    let blocks_html = (*blocks)
        ...
        .enumerate()
        .map(|(index, block)| {
            let is_active = *active_block_index == index;
