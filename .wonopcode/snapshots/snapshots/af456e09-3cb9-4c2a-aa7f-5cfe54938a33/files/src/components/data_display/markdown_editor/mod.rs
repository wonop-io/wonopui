...
pub fn markdown_editor<T: BlockTrait>(props: &MarkdownEditorProps<T>) -> Html {
    ...
    let on_drag_over = {
        ...
        Callback::from(move |(index, e): (usize, DragEvent)| {
            ...
            // Use current_target to get the element with the event listener, not a child
    ...
    let on_container_drop = {
        ...
        Callback::from(move |e: DragEvent| {
            ...
            if let Some(drag_index) = *dragging_index {
                // If we have an indicator, use it; otherwise drop at end
                let target_index = drop_indicator_index.unwrap_or(blocks.len());
            ...
            if let Some(target) = e.target() {
                if let Ok(element) = target.dyn_into::<web_sys::HtmlElement>() {
                    ...
                    if element.class_list().contains("markdown-editor-blocks") {
                        drop_indicator_index.set(Some(blocks.len()));
                    }
                }
            }
        })
