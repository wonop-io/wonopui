...
pub trait BlockTrait: Clone + PartialEq + 'static {
    ...
    /// 
    /// This method parses a markdown string and returns a vector of blocks.
    /// Each block type implementation should override this to handle its
    /// specific markdown syntax (e.g., headers, lists, code blocks).
    /// 
    /// The default implementation creates paragraph blocks for each
    /// non-empty line separated by blank lines.
    fn from_markdown(markdown: &str) -> Vec<Self> {
        // Default implementation: split by double newlines into paragraphs
        markdown
            .split("\n\n")
            .filter(|s| !s.trim().is_empty())
            .map(|s| Self::new_block_with_content(s.trim().to_string()))
            .collect()
    }
}
