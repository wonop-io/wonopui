...
pub fn editor_block<T: BlockTrait>(props: &EditorBlockProps<T>) -> Html {
    ...
    // Close dropdown when clicking outside - properly managed closure to avoid memory leak
    {
        ...
        // Store the closure in a ref to properly clean it up
        let closure_ref: Rc<RefCell<Option<Closure<dyn FnMut(web_sys::Event)>>>> =
            use_memo((), |_| Rc::new(RefCell::new(None)));

        use_effect_with(
            (*dropdown_open, closure_ref.clone()),
            move |(is_open, closure_ref)| {
                ...
                // Cleanup function
                let closure_ref = closure_ref.clone();
                move || {
                    if let Some(closure) = closure_ref.borrow_mut().take() {
                        if let Some(document) = web_sys::window().and_then(|w| w.document()) {
                            let _ = document.remove_event_listener_with_callback(
