...
pub fn iframe(props: &IframeProps) -> Html {
            ...
            if let Some(iframe) = iframe_ref.cast::<HtmlIFrameElement>() {
                if let Some(document) = iframe.content_document() {
                    ...
                    let onkeydown = {
                        ...
                        Closure::wrap(
                            Box::new(
                                move |event: web_sys::KeyboardEvent| match onkeydown.clone() {
                                    Some(onkeydown) => {
                    ...
                    let onmouseup = Closure::wrap(Box::new(move |event: web_sys::MouseEvent| {
                        ...
                        options.bubbles(true).cancelable(false);
                        let evt =
                            web_sys::CustomEvent::new_with_event_init_dict("mousemove", &options)
                                .expect("Failed to create event");
                        // evt.set_client_x(event.client_x() + bounding_client_rect.left() as i32);
                        // evt.set_client_y(event.client_y() + bounding_client_rect.top() as i32);
                        iframe
    ...
    // Links and style processing in use_effect
    let link_list = use_state(Vec::new);
    let preamble = use_state(String::new);

    {
        let link_list = link_list.clone();
        ...
        use_effect_with((), move |_| {
            ...
            let mut preamble_value = format!("<style>{}</style>", style);
            let mut link_list_value: Vec<Html> = Vec::new();
                    ...
                    if let Some(link_node) = links.get(i) {
                        if let Ok(link) = link_node.dyn_into::<web_sys::HtmlLinkElement>() {
                            ...
                            if href.ends_with(".css") {
                                link_list_value
                                    .push(html! { <link rel={rel.clone()} href={href.clone()} /> });

                                preamble_value = format!(
                                    "{}<link rel=\"{}\" href=\"{}\" />",
                                    preamble_value, rel, href
                                );
                            }
                        }
                    }
